 ...
 * - v1.1.0: Added Advanced Print Settings (Light-Off Delay, Rest Before Lift, UV PWM Power)
 * - v1.1.0: Added Dynamic G-Code parameter parsing
 */
...
byte Light_Off_Delay;       // Seconds to wait before turning on UV
byte Rest_Before_Lift;      // Seconds to wait after UV turns off
byte UV_Power_Percent;      // UV LED power (0-100%)
byte Override_GCODE;        // 0 = Use G-Code, 1 = Force EEPROM settings
...
void parse_gcode_parameters(String folderPath) {
  File gcodeFile = SD.open(folderPath + "/run.gcode"); 
  if (!gcodeFile) return;

  while (gcodeFile.available()) {
    String line = gcodeFile.readStringUntil('\n');
    
    // Stop reading once we hit the end of the header to save memory
...
void setup() {  
  ...
  // Initialize PWM for UV LED (Channel 0, 5000Hz, 8-bit resolution)
  // Change this:
  // ledcSetup(0, 5000, 8); 
  // ledcAttachPin(LED, 0); 
  // ledcWrite(0, 0); 
  ...
  // Advanced Print Parameters
  Light_Off_Delay = EEPROM.read(11);
  Rest_Before_Lift = EEPROM.read(12);
  UV_Power_Percent = EEPROM.read(13);
  ...
  // --- SAFEGUARDS FOR FIRST BOOT (Empty EEPROM = 255) ---
  bool eeprom_needs_commit = false;
  
  if (Light_Off_Delay == 255) { 
      Light_Off_Delay = 0; 
      EEPROM.write(11, 0); 
      eeprom_needs_commit = true; 
  }
  if (Rest_Before_Lift == 255) { 
      Rest_Before_Lift = 0; 
      EEPROM.write(12, 0); 
      eeprom_needs_commit = true; 
  }
  if (UV_Power_Percent == 255 || UV_Power_Percent == 0) { 
      UV_Power_Percent = 100; 
      EEPROM.write(13, 100); 
      eeprom_needs_commit = true; 
  }
  if (Override_GCODE == 255 || Override_GCODE > 1) { 
      Override_GCODE = 1; // Default to ON for backwards compatibility
      EEPROM.write(14, 1); 
      eeprom_needs_commit = true; 
  }
  ...
  if (digitalRead(buttonOK) == LOW) {
    switch (screen) {
      ...
      case 111: {
        ...
        // -------------------------------------------------------------------------------
        // Pre-Print Parameter Logic Decision
        // ------------------------------------------------------------------------------- 
        if (Override_GCODE == 0) {
            parse_gcode_parameters(foldersel_long); 
        } else {
            Regular_Exposure = EEPROM.read(3);
            Base_Exposure = EEPROM.read(2);
            Light_Off_Delay = EEPROM.read(11);
        ...
        while(!homing_canceled && !print_canceled){            
          ...
          // ---------------------------------------------------------
          // Advanced Timing & PWM Exposure Injection
          // ---------------------------------------------------------
          if (Light_Off_Delay > 0) delay(Light_Off_Delay * 1000); 
          ...
          // Calculate exact exposure time with transition logic
          float current_exposure_time = Regular_Exposure;
          if (current_layer <= Base_Layer) {
              current_exposure_time = Base_Exposure;
              Transition_Exposure = Base_Exposure;
          } else if (current_layer > Base_Layer && current_layer <= (Base_Layer + Transition_Layer)) {
              Transition_Exposure -= (Base_Exposure - Regular_Exposure) / (float)Transition_Layer;
              current_exposure_time = Transition_Exposure;
          }

          // Re-attach pin and apply dynamic PWM output
           // Change this:
          // ledcAttachPin(LED, 0); 
          ...
          delay(current_exposure_time * 1000);
          
          // Disable output safely
          ledcWrite(LED, 0);
          digitalWrite(LED, LOW);
          
          // Original turn_on_LED() is bypassed here to respect the custom PWM logic
          // turn_on_LED();  
          
          gfx1->fillScreen(BLACK);
          
          if (Rest_Before_Lift > 0) delay(Rest_Before_Lift * 1000);
          // ---------------------------------------------------------
      ...
      EEPROM.write(11, Light_Off_Delay);
      EEPROM.write(12, Rest_Before_Lift);
      EEPROM.write(13, UV_Power_Percent);
      EEPROM.write(14, Override_GCODE);
      EEPROM.commit(); 
      if(setting_item_updown == 1){
        setting_item ++;
        screen31UP();
